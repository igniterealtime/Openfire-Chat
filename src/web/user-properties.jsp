<%@ page import="org.jivesoftware.openfire.plugin.rawpropertyeditor.RawPropertyEditor"%>
...
<%
    ...
    if (!username.isEmpty() && username != null) {
        try {
            properties = RawPropertyEditor.self.getUserProperties(username);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    if (request.getParameter("save") != null && request.getParameter("save").equals("true")) {
        try {
            ...
            RawPropertyEditor.self.addProperties(username, propname2, propvalue2);
            properties = RawPropertyEditor.self.getUserProperties(username);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    if (request.getParameter("delete") != null && request.getParameter("delete").equals("true")) {
        try {
            RawPropertyEditor.self.deleteProperties(username, propname2);
            properties = RawPropertyEditor.self.getUserProperties(username);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
<meta name="subPageID" content="user-properties-plugin" />
...
<body onload="loaded();">
    ...
    <form action="user-properties.jsp<%="?username=" + username%>"
